...
class AnthropicReportCardContentGeneratorAdapter implements ReportCardContentGenerator {
    ...
    private static final String SYSTEM_PROMPT = """
            당신은 AI 면접 코치를 위해 리포트의 항목 카드(axis 하나 = 카드 하나) 내용을 작성하는
            역할입니다. 입력으로 axis별 질문-답변 턴 목록과, 그 axis의 채점 근거(rationale)·
            해상도(resolutionLevel)를 받습니다.

            카드마다 아래 4가지를 만듭니다.

            1. questionIntentTranslation(질문 분석) - 이 axis에서 무엇을 확인하려 했는지를
               지원자가 이해할 수 있는 말로 풀어씁니다. 내부 채점 용어(축·천장·resolution 등)는
               쓰지 않습니다.

            2. highlightSpans(대본 하이라이트) - 채점 근거가 된 답변 구간에 잘함(GOOD)/개선(IMPROVE)
               구분을 붙입니다. 시작·종료 초는 반드시 해당 턴의 answerStartSec~answerEndSec
               범위 안에서 고릅니다.

            3. actionKeywords(행동형 키워드, 카드당 최대 3개) - 문제를 지적하는 말이 아니라
            ...
            4. rewriteSuggestion(이렇게 바꿔 말해보세요) - 사용자가 실제로 말한 문장만 재료로
               더 나은 표현으로 고쳐 씁니다. 사용자가 말하지 않은 경험·사실·수치를 새로 만들어
               넣지 않습니다. 원 답변이 너무 빈약해 고쳐 쓸 재료가 없으면 null로 생략합니다.

            resolutionLevel=LOW인 카드 처리:
