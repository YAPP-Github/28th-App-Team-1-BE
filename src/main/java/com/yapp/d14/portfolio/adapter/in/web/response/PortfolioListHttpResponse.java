...
public record PortfolioListHttpResponse(
        @Schema(description = "이번 달 남은 재업로드(교체) 기회 여부. true면 1회 가능, false면 0회(업로드 시도 시 REPLACEMENT_LIMIT_EXCEEDED). 삭제 기회와는 독립적으로 집계됨. 포트폴리오 존재 여부와 무관하게 계정 단위로 결정됨")
        boolean replaceAvailable,

        @Schema(description = "재업로드가 막혀 있을 때 다시 가능해지는 시각(다음 달 1일 0시). 가능한 상태면 null")
        LocalDateTime nextAvailableAt,

        @Schema(description = "이번 달 남은 삭제 기회 여부. true면 1회 가능, false면 0회(삭제 시도 시 DELETE_LIMIT_EXCEEDED). 재업로드 기회와는 독립적으로 집계됨. 포트폴리오 존재 여부와 무관하게 계정 단위로 결정됨")
