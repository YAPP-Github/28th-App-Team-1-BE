...
| `job` | 직무/직군 참조 데이터 |
| `interview` | 면접 세션 진행, 질문 생성(preload), 답변 채점(axes/rubric), TTS, 면접 영상 보관 수명주기(`InterviewVideo`) |
| `ticket` | 이용권·예약(hold/commit/release) 관리 |
| `feedback` | 지인 피드백 공유 링크 발급·게스트(무인증) 평가 제출 |
| `common` | 공통 설정, 응답 포맷, 예외 기반 클래스, JWT 필터, `@CurrentUser` 리졸버 |

### 모듈 간 의존(크로스모듈 포트) 컨벤션

한 모듈이 다른 모듈의 데이터/기능이 필요하면, **데이터를 소유한(producer) 모듈이 자신의 `application/port/in/`에 `UseCase` 인터페이스를 정의**하고 자신의 `application/service/`에 package-private `@Service`로 구현합니다. 소비하는(consumer) 모듈은 이 인터페이스를 생성자로 직접 주입받아 씁니다 — consumer가 별도 out-port나 어댑터를 새로 만들지 않습니다.

예: `feedback` 모듈은 `interview.application.port.in.InterviewSessionOwnershipCheckUseCase`, `InterviewVideoQueryUseCase`, `InterviewVideoRetentionExtendUseCase`, `QuestionBoundaryQueryUseCase`와 `user.application.port.in.FindUserUseCase`를 그대로 주입받습니다. 모듈 간 예외는 모두 `BusinessException`을 상속하므로 별도 변환 없이 `GlobalExceptionHandler`가 처리합니다.

여러 개의 관련 동작을 하나의 UseCase 인터페이스로 묶을지(`InterviewVideoRetentionExtendUseCase`처럼 트리거별 메서드를 한 인터페이스에), 아니면 별도 인터페이스로 쪼갤지(`ticket`의 `TicketHoldUseCase`/`TicketCommitUseCase`/`TicketReleaseUseCase`처럼)는 "같은 로직을 파라미터만 다르게 재사용하는가" 대 "서로 다른 비즈니스 동작인가"로 판단합니다.
...
- AWS S3 (파일 저장) — 저장 경로 규칙·만료/삭제 정책은 `docs/policy/s3-policy.md`를 따릅니다. S3 관련 코드 작업 시 반드시 이 문서를 먼저 확인하세요.
...
# 지인 피드백(F4·G4) API 테스트용 완료된 세션·영상 픽스처 시딩 (-PuserId, -PuserName)
./gradlew seedFeedbackFixture
```
...
- CI(`deploy.yml`)는 `main` 브랜치 push 시 `./gradlew test`(단위 테스트만)를 통과해야 배포로 진행됩니다.
